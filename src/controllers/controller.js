...
const createIntern = async function (req, res) {
    try {
       ...
       if (regForEmail(email) == false) { return res.status(400).send({ status: false, message: "provide a valid email" }) }
        ...
        if (checkDuplicateemail) { return res.status(400).send({ status: false, message: "the email is exist please provide another email" }) }

        let checkDuplicatemobile = await internModel.findOne({ mobile: mobile })
        if (checkDuplicatemobile) { return res.status(400).send({ status: false, message: "the mobile No is exist please provide another Mobile No" }) }
